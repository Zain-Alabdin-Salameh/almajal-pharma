...
class AuthService {
  Future<LoginResponse> login(LoginReq req) async {
    try {
      ...
      return LoginResponse.empty("error : ${e}",
          status: 400, user: User.empty());
    }
  }
  ...
  Future<LoginResponse> updatePassword(
      String pass1, String pass2, String token) async {
    try {
      var data = await httpCall.post(ApiendPoints.changePass,
          data: {"current_password": pass1, "new_password": pass2},
          options: Options(headers: {
            HttpHeaders.authorizationHeader: "Bearer $token",
            HttpHeaders.acceptHeader: '*/*'
          }));
